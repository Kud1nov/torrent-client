...
func TestBuildTrackerURL(t *testing.T) {
	to := TorrentFile{
		AnnounceList: []string{"http://bttracker.debian.org:6969/announce"},
	...
	url, err := to.buildTrackerURL(to.AnnounceList[0], peerID, port)
...
func TestRequestPeers(t *testing.T) {
	...
	p, err := tf.requestPeers(tf.AnnounceList[0], peerID, port)
	assert.Nil(t, err)
